...
When we extend the dataset beyond 2005, the graph grows to large to process within resonable time on regular laptops. To benchmark further we would obvious need more hardware, this would only be a shortterm solution, Xeon platform servers with 16 ,32 or 128GB of memory would come up short, at this point implementation of our MapReduce algorithms would be a solution that we could benefit from. If we were to utilize our laptops as well as Xeon based servers to the full potiential we need to parallize the algorithms where possible. (Måske i konklusion i stedet?)
